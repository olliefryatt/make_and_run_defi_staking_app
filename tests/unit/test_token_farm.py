...
# Test set price feed contract on smart contract
def test_set_price_feed_contract():
    ...
    # Assert that address are correct
    assert token_farm.tokenPriceFeedMapping(dapp_token.address) == price_feed_address
    # Make sure other accounts can't call this function, i.e. only owner
    with pytest.raises(exceptions.VirtualMachineError):
       token_farm.setPriceFeedContract(
       dapp_token.address, price_feed_address, {"from": non_owner}
        )

# Check staking on the Smart Contract
def test_stake_tokens(amount_staked):
    ...
    # Assert
    # Amount we wanted to stake matches that in the smart contract 
    assert (token_farm.stakingBalance(dapp_token.address, account.address) == amount_staked)
    # Check that there should only be 1 uniqe token staked so far 
    assert token_farm.uniqueTokensStaked(account.address) == 1
    # Frist staker in this array should be the accounts 
    assert token_farm.stakers(0) == account.address
    # Return results as we can use this for more testing
...
def test_issue_tokens(amount_staked):
    ...
    # since the price of eth is ~3,000
    assert (
